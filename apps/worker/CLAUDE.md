...
- **KHÔNG phải workspace pnpm/turbo** (không có `package.json`). Chạy riêng bằng **`pnpm dev:worker`** (→ `uv run python -m fastcheck_worker`); kiểm tra bằng `pnpm worker:lint` / `worker:typecheck` / `worker:test`. Model pydantic **sinh từ JSON Schema** của contracts qua `pnpm worker:gen` (`_contracts_gen.py`) — đừng sửa tay. Golden set chạy `uv run pytest`.
